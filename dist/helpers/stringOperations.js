const getEscapedCSSClass = (className) => {
    return className
        .replace('(', '\\(')
        .replace(/_/g, '\\_')
        .replace(')', '\\)')
...
const getCSSValue = (className) => {
    return className
        .substring(className.indexOf('(') + 1, className.indexOf(')'))
        .replace(/_/g, ' ');
};
const getCSSProperty = (className) => {
    return className.substring(0, className.indexOf('('));
};
const formatCSSRule = (cssClass, property, value) => {
    return `.${cssClass}{\n\t${property}: ${value}\n}\n`;
};
const checkCorrectFormat = (className) => {
